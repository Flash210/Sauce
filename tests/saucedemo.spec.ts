...
test("Add one product and verify it in the cart", async ({ page }) => {
  ...
  const expectedName = await firstItem
    .locator(".inventory_item_name")
    .innerText();
  const expectedDesc = await firstItem
    .locator(".inventory_item_desc")
    .innerText();
  const expectedPrice = await firstItem
  ...
  await expect(cartItem.locator(".inventory_item_desc")).toHaveText(
    expectedDesc,
  );
